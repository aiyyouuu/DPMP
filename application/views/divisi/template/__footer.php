...
<script type="text/javascript">
    $(document).ready( function () {
				            ...
				            <select name="barang[]" id="barang`+count+`" data-row=`+count+` class="form-control barang">
		                      ...
		                      <?php foreach($barang as $value): ?>
		                        <?php if ($value['stock_barang'] > 0): ?>
		                          <option value="<?= $value['id_barang'] ?>" stockbarang`+count+`="<?= $value['stock_barang'] ?>" 
		                          ><?= ucwords($value['nama_barang']) ?></option>
		                        <?php endif ?>
		                      <?php endforeach ?>
		                    </select>
            			  </td>`;
	    ...
	    $('#saveTabel').on('click',function(){
	        ...
	        $.ajax({
	            method  : "GET",
	            url     : "<?= base_url('index.php/divisi/transaksi/insertPenjualan') ?>",
	            data    : { struk:struk, tanggal:tanggal, id_pelanggan:id_pelanggan, jumlah:jumlah, barang:barang },
	            success:function(data) {
	                alert(data);
	                window.location.replace('<?= base_url('index.php/divisi/transaksi') ?>')
	            }
	        });
	    });
